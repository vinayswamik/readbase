...
def complete_confluence_connection(
    ...
    if not code or not state or not confluence_state_cookie or state != confluence_state_cookie:
...
def disconnect_confluence_connection(user=Depends(require_authenticated_user)) -> dict:
    try:
        return disconnect_confluence(user.user_id)
    except ServiceError as exc:
        raise service_error_to_http(exc) from exc
...
def workspace_confluence_sources(workspace_id: str, user=Depends(require_authenticated_user)) -> dict:
    try:
...
def add_confluence_source(workspace_id: str, payload: AddWorkspaceConfluenceSpaceRequest, user=Depends(require_authenticated_user)) -> dict:
    try:
...
def remove_confluence_source(workspace_id: str, source_id: str, user=Depends(require_authenticated_user)) -> dict:
    try:
...
def sync_confluence_source(workspace_id: str, source_id: str, user=Depends(require_authenticated_user)) -> dict:
    try:
