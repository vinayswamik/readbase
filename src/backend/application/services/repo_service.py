...
def index_repository(repo_url: str, refresh: bool = False) -> dict:
    normalized_url = repo_url.strip()
    if not normalized_url:
        raise ValidationError("repo_url is required.")
    try:
        return index_repo(normalized_url, refresh=refresh)
    except RepoError as exc:
        raise ValidationError(str(exc)) from exc


def index_local_repository(local_path: str, refresh: bool = False) -> dict:
    normalized_path = local_path.strip()
    if not normalized_path:
        raise ValidationError("local repository path is required.")
    try:
        return index_local_repo(normalized_path, refresh=refresh)
    except RepoError as exc:
        raise ValidationError(str(exc)) from exc
