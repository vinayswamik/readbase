...
export async function reloadConnectorConnection(connectorId: ConnectorId, loads: ConnectorLoads) {
  switch (connectorId) {
    case "github":
...
type UseWorkspaceConnectorHandlersIntegrationsArgs = {
  workspace: Workspace;
  handleApiError: (error: unknown, setMessage?: (message: string) => void) => void;
  loads: ConnectorLoads;
  onWorkspaceSourcesChanged?: () => void;
...
// Kept private to this module: surfaced via useWorkspaceConnectorHandlers above.
function useWorkspaceConnectorHandlersIntegrations({
  workspace,
  handleApiError,
  loads,
  onWorkspaceSourcesChanged,
  ...
  function handleSlackConnect() {
    void startOAuthFlow(
      `/api/me/integrations/slack/start?${new URLSearchParams({
  ...
  async function handleSlackDisconnect(teamId?: string) {
    ...
    try {
      ...
      setSlackChannels([]);
      setConnectorStatus(teamId ? "Slack workspace removed." : "Slack disconnected.");
      notifyWorkspaceSourcesChanged();
    } catch (error) {
      handleApiError(error, setConnectorError);
    } finally {
